describe("Negative Scenarios - Wrong Data", () => {
  beforeEach(() => {
    ...
    cy.fixture("response-schema/empty-fields-response").as(
      "emptyResponseSchema"
    );
    cy.fixture("response-schema/value-errors-response").as(
  ...
  it("Create new user - Empty fields", function () {
    ...
    // Create a new user
    cy.createNewUser(userNoValues).then((response) => {
      // Response validation
      ...
      expect(response.body[2].message).to.be.eq(
